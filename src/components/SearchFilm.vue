...
const filteredFilms = computed(() => {
  return films.value.filter((film) => {
    return film.title.toLowerCase().includes(query.value.toLowerCase());
  })
})
...
const Etoiles = (metascore) => {
  const score = metascore
  if (score >= 80) {
    return '★★★★★'
  } else if (score >= 60) {
    return '★★★★'
...
<template>
  <div id="search-film">
    ...
    <p v-if="numberResults === 0">Aucun film trouvé</p>
    <p v-else>{{ numberResults }} films trouvés</p>

    <ul class="films" v-for="film in filteredFilms" :key="film.title">
      <li class="film card">
        <img class="poster" :src="film.poster" />
        <p class="title">
          {{ film.title }}
          <span class="rating">{{ Etoiles(film.metascore) }}</span>
        </p>
        <dl>
          <dt>Release date</dt>
          <dd>{{ film.released }}</dd>
